...
export default function BettingBoard({ me, locked, onPlaceBet, onClearBet, onConfirm }: {
  ...
  function addChip(type: BetType) {
    if (locked || chipValue > remaining) return;
    onPlaceBet(type, (betsByType.get(type) || 0) + chipValue);
  }
  ...
  function betButton(type: BetType, main = false) {
    ...
    return (
      <div key={type} className="relative min-w-0">
        <button type="button" data-testid={`bet-${type}`} disabled={locked} onClick={() => addChip(type)} className={`${main ? 'bet-main-button' : 'bet-option-button'} w-full active:scale-[0.97] transition disabled:opacity-45`}>
