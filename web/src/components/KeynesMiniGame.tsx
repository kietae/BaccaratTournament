...
export default function KeynesMiniGame({ state, admin = false, onSubmit, onReveal }: {
  ...
  const [value, setValue] = useState(game.myNumber ?? 50);
  ...
  if (game.status === 'revealed') {
    return (
      <section className="w-full rounded-3xl border border-violet-400/30 bg-[radial-gradient(circle_at_top,#39205e,#100b1b_68%)] p-5 lg:p-8 text-center">
        ...
        <h2 className="mt-2 text-3xl lg:text-5xl font-black text-white">Beauty Contest</h2>
        <p className="mt-1 text-sm font-bold text-violet-200">평균의 2/3 결과</p>
        <div className="mt-5 flex justify-center gap-3 text-sm"><span className="rounded-full bg-black/30 px-4 py-2 text-zinc-300">평균 <b className="text-white">{game.average?.toFixed(2)}</b></span><span className="rounded-full bg-amber-400/15 px-4 py-2 text-amber-200">목표값 <b>{game.target?.toFixed(2)}</b></span></div>
        ...
        <div className="mx-auto mt-6 max-h-[45vh] max-w-2xl space-y-2 overflow-y-auto pr-1">
          {game.results.map((result) => <div key={result.playerId} className={`grid grid-cols-[3rem_1fr_auto_auto] items-center gap-3 rounded-xl border px-4 py-3 text-left ${result.playerId === state.me?.id ? 'border-amber-300 bg-amber-300/15' : result.rank === 1 ? 'border-violet-300/50 bg-violet-300/10' : 'border-white/10 bg-black/20'}`}><span className="text-xl font-black text-amber-300">{result.rank}위</span><span className="truncate font-bold text-white">{result.nickname}</span><span className="font-mono text-lg text-violet-200">{result.value}</span><span className="text-xs text-zinc-400">차이 {result.distance.toFixed(2)}</span></div>)}
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="w-full rounded-3xl border border-violet-400/30 bg-[radial-gradient(circle_at_top,#39205e,#100b1b_68%)] p-5 lg:p-8 text-center">
      <p className="text-xs font-bold tracking-[0.28em] text-violet-300">MINI GAME</p>
      <h2 className="mt-2 text-3xl lg:text-5xl font-black text-white">Beauty Contest</h2>
      <p className="mx-auto mt-3 max-w-xl text-sm text-zinc-300">0~100 사이 숫자 하나를 적어라. 전체 평균의 2/3에 가장 가까운 사람이 우승.</p>
      <p className="mx-auto mt-2 max-w-xl text-xs text-zinc-400">동점이면 최종 숫자를 먼저 제출한 사람이 우선합니다. 10명 이상 참여하면 더 재미있습니다.</p>
      ...
      {admin ? (
        ...
        <div className="mx-auto mt-6 flex max-w-sm flex-col items-center gap-3">
          <input aria-label="미니게임 숫자" type="number" inputMode="numeric" min={0} max={100} step={1} value={value} onChange={(event) => setValue(Number(event.target.value))} className="w-40 rounded-2xl border border-violet-300/40 bg-black/35 px-4 py-3 text-center text-4xl font-black text-white outline-none focus:border-violet-200" />
