...
function BaccaratBroadcast({state}:{state:TableState}){
  ...
  return <div className="grid h-full grid-rows-[13rem_1fr] gap-4"><div className="grid grid-cols-[1.3fr_.7fr] gap-4"><section className="overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4"><h2 className="mb-2 text-xl font-black">바카라 매</h2><BigRoadGrid road={state.bigRoad}/></section><PublicLeaderboard state={state} limit={5}/></div><section className="grid min-h-0 grid-cols-[1fr_1.25fr_1fr] items-center gap-5 rounded-3xl border border-emerald-600/30 bg-[radial-gradient(circle_at_top,#16543d,#08251a_70%)] p-5"><ScreenCards label="PLAYER" cards={state.cards.filter((card)=>card.side==='player')}/><div className="flex h-full items-center justify-center">{active?.needsSqueeze&&(state.phase==='squeeze'||state.phase==='extra-card')?<div className="flex h-full flex-col items-center"><b className="mb-2 text-xl text-amber-200">{state.squeezerNickname||'딜러'} 스퀴즈</b><div className="h-[calc(100%-2rem)] aspect-[11/16] overflow-hidden rounded-2xl border border-amber-300/40"><SqueezeCanvas mode="remote" revealed={active.revealed} rank={active.rank} suit={active.suit} remoteEdge={active.edge} remotePct={active.pct} remoteGrip={active.grip}/></div></div>:resultVisible&&state.result?<RoundResultCallout result={state.result} large/>:<div className="text-center"><p className="text-5xl font-black">{PHASE[state.phase]}</p><p className="mt-3 text-xl text-emerald-100/60">ROUND {state.roundNo}</p></div>}</div><ScreenCards label="BANKER" cards={state.cards.filter((card)=>card.side==='banker')}/></section></div>;
}

function ScreenCards({label,cards}:{label:'PLAYER'|'BANKER';cards:CardView[]}){const prefix=label==='PLAYER'?'P':'B';return <div className="text-center"><h3 className={`mb-3 text-3xl font-black ${label==='PLAYER'?'text-blue-200':'text-red-200'}`}>{label}</h3><div className="grid grid-cols-2 justify-items-center gap-3">{[1,2,3].map((number,index)=>{const card=cards.find((item)=>item.cardId===`${prefix}${number}`);return <div key={number} className={index===2?'col-span-2':''}>{card?.dealt?<CardSlot card={card} scale={1.5}/>:<EmptyCardSlot orientation={index===2?'horizontal':'vertical'} scale={1.5}/>}</div>;})}</div></div>;}
