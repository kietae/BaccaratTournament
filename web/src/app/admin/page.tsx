...
export default function AdminPage() {
  ...
  return <main className={`flex-1 w-full mx-auto p-3 lg:p-4 ${state.status === 'active' ? 'h-[100dvh] overflow-hidden flex flex-col' : ''} ${presentation ? 'max-w-none' : 'max-w-7xl'}`}>
    ...
    {state.status === 'lobby' ? <Lobby state={state} qr={qr} onStart={startTournament} onStartMiniGame={startMiniGame} onRevealMiniGame={revealMiniGame} error={error} /> : state.status === 'finished' ? <FinalLeaderboard state={state} onPrepareNew={prepareNewTournament} error={error} /> : <LiveDashboard state={state} />}
  </main>;
}
...
function Lobby({ state, qr, onStart, onStartMiniGame, onRevealMiniGame, error }: { state: TableState; qr: string | null; onStart: () => void; onStartMiniGame: (type: 'beauty-contest' | 'lowest-unique') => void; onRevealMiniGame: () => void; error: string | null }) {
  if (state.miniGame.status !== 'idle') return <div className="flex flex-col gap-5"><KeynesMiniGame state={state} admin onReveal={onRevealMiniGame} />{state.miniGame.status === 'revealed' && <button data-testid="start-tournament" onClick={onStart} disabled={state.playerCount === 0} className="self-center rounded-xl bg-amber-500 px-8 py-4 text-lg font-black text-zinc-950 disabled:opacity-40">바카라 토너먼트 시작</button>}{error && <p className="text-center text-sm text-red-400">{error}</p>}</div>;
